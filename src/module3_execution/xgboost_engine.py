...
def execute_classical_fallback(X_train: pd.DataFrame, y_train: pd.Series, X_test: pd.DataFrame) -> np.ndarray:
    ...
    #
    # BUGFIX: removed `use_label_encoder=False`. That argument was deprecated
    # in XGBoost 1.6 and was removed entirely in 2.x -- on any reasonably
    # current XGBoost install this raises
    # `TypeError: __init__() got an unexpected keyword argument 'use_label_encoder'`
    # the moment this constructor runs, before training even starts.
